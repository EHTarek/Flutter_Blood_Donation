...
class DbHelper{
  ...
  static Future<List<UserReviews>> getRatingsByDonorId(int id) async {
    final db = await open();
    final mapList = await db.query(tableUserReview,
      where: '$tblUserReviewColDonorId = ?', whereArgs: [id],);
    return List.generate(mapList.length, (index) =>
...
/*  static Future<List<UserReviews>> getReviewsByUserId(int id) async {
  ...
  static Future<bool> didUserRate(int donorId, int userId) async {
    final db = await open();
    final mapList = await db.query(tableUserReview,
      where: '$tblUserReviewColDonorId = ? and $tblUserReviewColId = ?', whereArgs: [donorId, userId],);
    return mapList.isNotEmpty;
  }
  static Future<int> updateRating(UserReviews userReviews) async {
    final db = await open();
    return db.update(tableUserReview, userReviews.toMap(),
      where: '$tblUserReviewColDonorId = ? and $tblUserReviewColId = ?', whereArgs: [userReviews.donorId, userReviews.userId],);
  }
}
